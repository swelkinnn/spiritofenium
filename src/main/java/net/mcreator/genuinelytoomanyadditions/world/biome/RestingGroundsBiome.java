...
public class RestingGroundsBiome extends SoeModElements.ModElement {
	public static Biome biome;
	public RestingGroundsBiome(SoeModElements instance) {
		super(instance, 329);
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-13421773).setWaterColor(-16777165).setWaterFogColor(-16777165)
						.withSkyColor(-13421773).withFoliageColor(-16777216).withGrassColor(-13421773).build();
				...
				mobSpawnInfo.withSpawner(EntityClassification.MONSTER, new MobSpawnInfo.Spawners(TheLostEntity.entity, 20, 1, 4));
				biome = new Biome.Builder().precipitation(Biome.RainType.SNOW).category(Biome.Category.PLAINS).depth(0.1f).scale(0.2f)
						.temperature(-1f).downfall(1f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("soe:resting_grounds"));
			}
		}
	}
	@Override
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("resting_grounds_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("resting_grounds_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
		@Override
		protected TreeDecoratorType<?> func_230380_a_() {
	...
	private static class CustomCocoaTreeDecorator extends CocoaTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("resting_grounds_ctd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
