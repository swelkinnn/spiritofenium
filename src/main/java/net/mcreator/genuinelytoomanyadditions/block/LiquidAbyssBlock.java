...
public class LiquidAbyssBlock extends SoeModElements.ModElement {
	...
	public void initElements() {
		fluidproperties = new ForgeFlowingFluid.Properties(() -> still, () -> flowing,
				FluidAttributes.builder(new ResourceLocation("soe:blocks/abyss"), new ResourceLocation("soe:blocks/abyss")).luminosity(0)
						.density(5000).viscosity(10000).temperature(0).rarity(Rarity.COMMON)).explosionResistance(100f).canMultiply().tickRate(2)
								.levelDecreasePerBlock(1).slopeFindDistance(4).bucket(() -> bucket).block(() -> block);
		still = (FlowingFluid) new CustomFlowingFluid.Source(fluidproperties).setRegistryName("liquid_abyss");
		flowing = (FlowingFluid) new CustomFlowingFluid.Flowing(fluidproperties).setRegistryName("liquid_abyss_flowing");
		...
		elements.items.add(() -> new BucketItem(still,
				new Item.Properties().containerItem(Items.BUCKET).maxStackSize(1).group(GTMBlocksItemGroup.tab).rarity(Rarity.COMMON))
	...
	public static abstract class CustomFlowingFluid extends ForgeFlowingFluid {
		...
		public IParticleData getDripParticleData() {
			return NowhereSoulParticle.particle;
		}
